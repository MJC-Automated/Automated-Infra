...
2. **Run the playbook from the repository root or `ansible/` with the shared inventory model.**
...
5. **Ansible Config:** The tracked `dev` inventory is the source template. Local `ansible.cfg` files may point at a generated environment to keep commands short, while explicit `-i` flags remain the portable form.
...
```bash
cd ~/IaC-Homelab/ansible/user-man

# Create or update users using this directory's ansible.cfg inventory and vault defaults
...
- Service accounts can be approved for the unmanaged-account baseline while still being excluded from SSH login policy with `sshd_allowlist_exclude_users`.
- By default, `oracle`, `wlogic`, `jenkins`, and `zimbra` are excluded from generated `AllowUsers` entries even when they are approved unmanaged accounts.
- Repository rule: committed `allowed_unmanaged_accounts_by_host` entries intentionally cover only the tracked `dev-*` hosts.
- For generated environments clone the tracked `dev` group-vars pattern and extend host-specific exceptions locally or in environment-specific inventory/group vars instead of adding more tracked non-dev hostnames here.
...
Inventory is generated by Terraform under `../../inventories/<env>/inventory.ini` and should not be edited by hand. The tracked `dev` environment is the source model; generated environments should clone its group-vars/service layout. The `ansible.cfg` in this project may point at the active generated inventory and also includes `../../inventories/aliases.ini` for app-specific group aliases.
...
Current tracked `dev` desired topology spans 18 hosts:
...
The idempotency snapshot below predates the CI/CD and Kubernetes expansion and covers only the original 10 service hosts. It does not establish user-management behavior on the eight later additions.
...
- **Service account is approved but cannot SSH:** check `sshd_allowlist_exclude_users`. Accounts in that list are removed from generated `AllowUsers` even if they are present in `allowed_unmanaged_accounts`.
