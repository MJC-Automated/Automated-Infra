...
- `.env.example`: optional secret and non-secret overrides
...
```bash
cd ~/IaC-Homelab/ansible/bootstrap_playbooks/freeipa
ansible-playbook main.yml
```

## Run Order

1. Run `ansible/time_sync/main.yml` first so the FreeIPA host and all Kerberos clients are on repo-managed Chrony sources.
...
- Uses injected `freeipa_ds_password` and `freeipa_admin_password` when supplied.
- Otherwise generates distinct 24-character hexadecimal credentials under `files/`; the directory is ignored, mode `0700`, and each secret is mode `0600`. This length is intentionally conservative for the tested OL9 FreeIPA/Dogtag PKCS#12 path.
- Non-secret identity, DNS, and firewall behavior is repository-backed in `group_vars/freeipa_servers.yml`; `.env` is optional.
...
- Host A/AAAA for the FreeIPA server FQDN, for example `public-freeipa-02.example.internal -> 198.51.100.45`
- Host A/AAAA for `ipa-ca.<domain>`, for example `ipa-ca.example.internal -> 198.51.100.45`
- SRV records:
...
host-record=public-freeipa-02.example.internal,198.51.100.45
host-record=ipa-ca.example.internal,198.51.100.45
...
```bash
dig @198.51.100.46 +short A ipa-ca.example.internal
dig @198.51.100.46 +short SRV _ldap._tcp.example.internal
dig @198.51.100.46 +short TYPE256 _kerberos.example.internal
```
