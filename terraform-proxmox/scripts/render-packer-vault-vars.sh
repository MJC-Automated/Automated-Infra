...
# Match vault-auth.sh / with-vault-token.sh so Packer credential renders work
# against the local self-signed Vault listener without a manual VAULT_CACERT export.
...
Environment:
  ...
  ENV_FILE              Optional dotenv path (default: <repo>/.env); loaded only
                        when VAULT_ADDR or token/AppRole identity is missing.
EOF
}
