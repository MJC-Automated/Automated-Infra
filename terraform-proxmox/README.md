...
- `TF_VAR_vault_address=https://198.51.100.19:8200`
- `VAULT_ADDR=https://198.51.100.19:8200`
- `PROXMOX_USER=root`
- `PROXMOX_HOST_DEV=198.51.100.20`
- `PROXMOX_HOST_PROD=198.51.100.21`
- `PROXMOX_HOST_TESTING=198.51.100.22`
...
# This repo documents a LAN default of 198.51.100.19 for dev examples.
VAULT_ADDR=https://198.51.100.19:8200
TF_VAR_vault_address=https://198.51.100.19:8200
```
...
```bash
make vault-mode-lan ENVIRONMENT=dev VAULT_LAN_IP=198.51.100.19 VAULT_LAN_HOST=198.51.100.19

# Regenerate Vault cert only (adds SANs for 127.0.0.1 + LAN host/IP)
make vault-tls-regenerate VAULT_LAN_IP=198.51.100.19 VAULT_LAN_HOST=198.51.100.19
```
...
# PROXMOX_HOST_QA=198.51.100.18
# PROXMOX_NODE_QA=proxmox
# ANSIBLE_HOST_QA=198.51.100.23
# NETWORK_CIDR_QA=198.51.100.0/24
# NETWORK_GW_QA=198.51.100.24
...
make env-discover ENVIRONMENT=qa PROXMOX_HOST=198.51.100.18

# Scaffold using discovery (default AUTO_DISCOVER=true)
make env-template \
  ENVIRONMENT=qa \
  TEMPLATE_ENV=dev \
  PROXMOX_HOST=198.51.100.18 \
  AUTO_DISCOVER=true
```
...
- `ip=203.0.113.0/24,gw=198.51.100.12`
- `ip=192.0.2.0/24,gw=198.51.100.12`
- `ip=198.51.100.0/24,gw=198.51.100.12`
- `ip=203.0.113.0/24,gw=198.51.100.12`
- `ip=192.0.2.0/24,gw=198.51.100.12`
...
Concrete example (dev-like stack on `198.51.100.22` with IPs `198.51.100.25-130`):

```bash
make env-template ENVIRONMENT=testing TEMPLATE_ENV=dev PROXMOX_HOST=198.51.100.22 ENV_TEMPLATE_FORCE=true
...
Concrete example (`example` cloned from the tracked `dev` scaffold, subnet `198.51.100.0/24`, Proxmox host `198.51.100.21`, control node `198.51.100.26`):
...
```bash
make env-template ENVIRONMENT=qa PROXMOX_HOST=198.51.100.18 \
  PROXMOX_NODE=proxmox STORAGE_POOL=local-zfs DATA_STORAGE=local-lvm \
  NETWORK_CIDR=198.51.100.0/24 NETWORK_GW=198.51.100.24 ENV_TEMPLATE_FORCE=true
```
