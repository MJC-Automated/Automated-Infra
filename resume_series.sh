...
RUN_ID="$(date +%Y%m%d-%H%M%S)-RESUME"
...
export ANSIBLE_BECOME_TIMEOUT="${ANSIBLE_BECOME_TIMEOUT:-120}"
ENVIRONMENT="${ENVIRONMENT:-dev}"
STRICT_IDEMPOTENCY="${STRICT_IDEMPOTENCY:-false}"
TCP_WAIT_TIMEOUT="${TCP_WAIT_TIMEOUT:-1800}"
LOGIN_WAIT_TIMEOUT="${LOGIN_WAIT_TIMEOUT:-2700}"
...
# Terraform steps skipped in resume
progress "Resuming series: Terraform and host-readiness already verified."
...
progress "Waiting 10 seconds before next playbook..."
sleep 10
...
run_step "verify_weblogic12_services_and_ports" bash -lc "
  ...
  code=\$(curl -k -sS -o /dev/null -w '%{http_code}' --max-time 10 http://${HOST_WLS12_IP}:8006/console || true)
